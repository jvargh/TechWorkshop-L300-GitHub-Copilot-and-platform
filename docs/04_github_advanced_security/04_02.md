...
2. Open the `src/appsettings.json` file. Add the following placeholder settings (do **not** use real secrets) and save the changes:

        ```json
        "secrets": {
            "users": [
                { "username": "admin", "password": "<placeholder-password>" }
            ],
            "database": {
                "connectionString": "Server=tcp:placeholder.database.windows.net,1433;Initial Catalog=placeholder;Persist Security Info=False;User ID=sa;Password=<placeholder-password>;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;"
            }
        }
        ```

        Replace each `<placeholder-...>` with non-sensitive dummy text (for example `fake-secret`) so the values clearly are not production credentials.

3. Open the `/src/Controllers/CartController.cs` file. Add the following **dummy** variables to the class and save the changes:

    ```csharp
    public class CartController : Controller
    {
        string stripe_key = "<placeholder-stripe-key>";
        string admin_password = "<placeholder-admin-password>";

        // ...
    }
    ```

    Keep the placeholders obviously fake (for example `demo-stripe-key`) so push protection does not detect real secrets.
